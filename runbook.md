...
9. Verify connection with command: `ansible db_servers -m ping`
   Expected output:
   ```
   db1 | SUCCESS => {
       "ansible_facts": {
...
## Verification
- Run `docker ps` to verify that 2 healthy containers are running
- Check backups: `ls -la /srv/backups/postgres/` after the cron job executes
